...
WITH active_users AS (
    ...
    WHERE wr.status IN ('solved', 'failed')
      AND NOT EXISTS (
          SELECT 1
          FROM public.flagged_users f
          WHERE f.user_id = wr.user_id
            AND f.draw_date = w.draw_date
      )
)
SELECT
    u.id AS user_id,
    ...
    COALESCE(sum(wr.nb_tries) FILTER (WHERE wr.status IN ('solved', 'failed')), 0) AS total_tries,
...
GROUP BY u.id, u.login, u.avatar_url, u.privacy_enabled, au.draw_date
HAVING sum(CASE WHEN wr.status IN ('solved', 'failed') THEN 1 ELSE 0 END) > 0;
