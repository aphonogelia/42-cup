/**
 * NOT USED - can be deleted - keeping it as helper for bot dev
 * Seeds the daily draw in the `words` table for a Berlin date. The active
 * 5 answers are stored per day, so the game can switch to a new draw at
...
const count = countFlagIndex !== -1 ? Number(args[countFlagIndex + 1]) : 5;
